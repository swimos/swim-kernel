...
use swim_mem::alloc::{Slab, Pool, StowFrom};
use swim_collections::hash_trie::HashTrieSet;

macro_rules! bench_update {
    ($bench:expr, $hold:expr, $n:expr) => ({
        let mut xs = HashTrieSet::<u32>::hold_new($hold);
        for k in 0..$n {
            xs.insert(k).unwrap();
        }
        let mut ys = HashTrieSet::stow_from(xs, $hold);
        let mut i = 0;
        $bench.iter(|| {
            ys.insert(i).unwrap();
...
fn bench_update_depth4(bench: &mut Bencher) {
    static mut TEST_HUNK: [u8; 128*1024*1024] = [0; 128*1024*1024];
    let slab = Slab::new(unsafe { Block::from_slice(&mut TEST_HUNK) }, 4096);
    let pool = &Pool::new(&slab);
